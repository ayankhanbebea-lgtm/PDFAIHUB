          ...
          <div className="section-container">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {blogPosts.map((post) => (
                <article
                  key={post.slug}
                  className="group flex flex-col bg-[#161B22] border border-[#1F2937] hover:border-[#10B981]/30 hover:shadow-card-hover rounded-2xl overflow-hidden transition-all duration-300 hover:-translate-y-1"
                >
                  {/* Card Content */}
                  <div className="p-6 flex-1 flex flex-col justify-between">
                    <div>
                      ...
                      <div className="flex items-center gap-3 text-xs mb-4">
                        <span className="bg-[#10B981]/10 text-[#10B981] px-2.5 py-1 rounded-full font-medium">
                          {post.category}
                        </span>
                      ...
                      <Link href={`/blog/${post.slug}`} className="block">
                        <h2 className="text-xl font-bold text-white mb-3 group-hover:text-[#10B981] transition-colors leading-snug">
                          {post.title}
                        </h2>
                      </Link>
                    ...
                    <div className="pt-4 border-t border-[#1F2937]/60 flex items-center justify-between">
                      ...
                      <Link
                        href={`/blog/${post.slug}`}
                        className="flex items-center gap-1 text-xs font-semibold text-[#10B981] group-hover:underline"
                      >
                        Read More
